...
local function applyFlyJumpToPlayer(p)
	if p == nil or not p.isValid then
		return
	end

	local isJumpHeld = (p.keys.jump == KEYS_DOWN) or (p.keys.altJump == KEYS_DOWN) or (p.jumpKeyPressing) or (p.altJumpKeyPressing)
	if not isJumpHeld then
		return
	end

	p.jumpKeyPressing = true
	p.altJumpKeyPressing = true
	p.speedY = -8.5
	p:mem(0x1C, FIELD_WORD, -1)
end

function playerManager.onInputUpdate()
	for _, p in ipairs(Player.get()) do
		applyFlyJumpToPlayer(p)
	end
